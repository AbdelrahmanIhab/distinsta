#!/usr/bin/env python3
"""
Aggressive Load Balancing Stress Test
Tests system with 1000 images of varying sizes
"""
...
NUM_UPLOADS = 1500
USERNAME = "stresstest"
MIN_IMAGE_SIZE = 1600    # pixels (1600x1600 = ~5 MB minimum)
MAX_IMAGE_SIZE = 3500    # pixels (3500x3500 = ~25 MB maximum)
...
def create_test_image(path, size, index):
    ...
    # Create image with random noise - this prevents compression
    # Each pixel is random, making PNG compression ineffective
    import numpy as np
    ...
    text = f"Image #{index} - {size}x{size}"
...
def create_test_files(test_dir, num_files):
    ...
    print(f"Creating {num_files} test images with varying sizes...")
    print(f"  Size range: {MIN_IMAGE_SIZE}x{MIN_IMAGE_SIZE} to {MAX_IMAGE_SIZE}x{MAX_IMAGE_SIZE}")

    start_time = time.time()

    # Categorize by size for statistics (in MB)
    small_count = 0   # 1600-2000px (~5-8 MB)
    medium_count = 0  # 2000-2500px (~8-13 MB)
    large_count = 0   # 2500-3000px (~13-18 MB)
    xlarge_count = 0  # 3000-3500px (~18-25 MB)

    for i in range(1, num_files + 1):
        # Vary image sizes - distributed across size ranges
        rand_val = i % 4
        if rand_val == 0:
        ...
        image_path = os.path.join(test_dir, f"stress_image_{i}.png")
    ...
    print(f"    - Small  (1600-2000px, ~5-8 MB):   {small_count:4d} images")
    print(f"    - Medium (2000-2500px, ~8-13 MB):  {medium_count:4d} images")
    print(f"    - Large  (2500-3000px, ~13-18 MB): {large_count:4d} images")
    print(f"    - XLarge (3000-3500px, ~18-25 MB): {xlarge_count:4d} images")
    print(f"  Total size: {total_size_gb:.2f} GB ({total_size_mb:.0f} MB)")
    print(f"  Average size: {total_size_mb/num_files:.1f} MB per image")
    print(f"  Network traffic (3x broadcast): ~{total_size_gb*3:.1f} GB\n")
...
def run_stress_test(commands):
    ...
    print_header("Starting Aggressive Stress Test")
    print(f"This will take a while... uploading {NUM_UPLOADS} images")
    ...
    try:
        output, _ = process.communicate(input=commands, timeout=3600)  # 60 minute timeout (1 hour)
    except subprocess.TimeoutExpired:
        process.kill()
        output, _ = process.communicate()
        print("\n⚠ WARNING: Test timed out after 60 minutes")

    elapsed = time.time() - start_time

    print(f"\n✓ Test completed in {elapsed:.1f} seconds ({elapsed/60:.1f} minutes)")
...
def analyze_results(output, num_uploads, elapsed_time):
    ...
    print("-" * 60)
    print(f"Node 1: {node1_processed:4d} processed, {node1_declined:4d} declined")
    print(f"Node 2: {node2_processed:4d} processed, {node2_declined:4d} declined")
    print(f"Node 3: {node3_processed:4d} processed, {node3_declined:4d} declined")
    print()

    total_processed = node1_processed + node2_processed + node3_processed
    total_received = node1_processed + node1_declined

    print(f"Total processed:  {total_processed}")
    print(f"Requests per node: {total_received} (client broadcasts to all)")
    print()
...
def evaluate_load_balancing(results, num_uploads):
    ...
    if nodes_used == 3:
        ...
        if balanced:
            print("✓ PASS: Distribution is well-balanced (>20% per node)")

            # Check for excellent distribution (25-40% per node)
            min_excellent = results['total'] * 0.25
            max_excellent = results['total'] * 0.40

            excellent = all([
                min_excellent <= results['node1'] <= max_excellent,
                min_excellent <= results['node2'] <= max_excellent,
                min_excellent <= results['node3'] <= max_excellent
            ])

            if excellent:
                print("✓ EXCELLENT: Distribution is within 25-40% range per node")
        else:
            print("⚠ WARNING: Distribution is uneven")
            print("  Some nodes handling <20% of load")
        ...
        print(f"\n  All requests going to: {active_node}")
    ...
    if success_rate == 100:
        ...
        print(f"⚠ ACCEPTABLE: {success_rate:.1f}% success rate")
    else:
        print(f"✗ POOR: {success_rate:.1f}% success rate")

    if results['failures'] > 0:
        print(f"\n  {results['failures']} uploads failed")
        print("  Check logs for error messages")

    print()

def generate_report(results, elapsed_time, num_uploads):
    ...
    if total > 0:
        print(f"  - Node 1: {results['node1']/total*100:5.1f}% ({results['node1']} requests)")
        print(f"  - Node 2: {results['node2']/total*100:5.1f}% ({results['node2']} requests)")
        print(f"  - Node 3: {results['node3']/total*100:5.1f}% ({results['node3']} requests)")
    print()
    ...
    if nodes_used == 3 and success_rate >= 95:
        print("  ✓✓✓ SYSTEM PASSED ALL TESTS")
        print("  Load balancing and fault tolerance working correctly!")
    elif nodes_used == 3:
        print("  ✓ SYSTEM PASSED (with warnings)")
        print("  Load balancing works but some uploads failed")
    elif nodes_used == 2:
        print("  ⚠ SYSTEM PARTIALLY WORKING")
        print("  One node may be down or unreachable")
    else:
        print("  ✗ SYSTEM FAILED")
...
def main():
    ...
    print_header("AGGRESSIVE LOAD BALANCING STRESS TEST")
    print(f"Configuration:")
    print(f"  - Number of uploads: {NUM_UPLOADS}")
    print(f"  - Username: {USERNAME}")
    print(f"  - Image size range: {MIN_IMAGE_SIZE}x{MIN_IMAGE_SIZE} to {MAX_IMAGE_SIZE}x{MAX_IMAGE_SIZE}")
    print(f"\nThis test will:")
    print(f"  1. Generate {NUM_UPLOADS} unique images of varying sizes")
    print(f"  2. Upload all images through the distributed system")
    print(f"  3. Analyze load distribution across all nodes")
    print(f"  4. Calculate performance metrics")
    print()
    ...
    try:
        ...
        # Run stress test
        output, elapsed_time = run_stress_test(commands)

        # Save output to file for debugging
        log_file = "aggressive_stress_test_output.log"
        ...
        generate_report(results, elapsed_time, NUM_UPLOADS)
